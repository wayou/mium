...
// https://github.com/DefinitelyTyped/DefinitelyTyped/issues/30070#issuecomment-434034763
declare module 'react' {
  // React 16.6

  function memo<Props>(component: React.StatelessComponent<Props>):
      React.StatelessComponent<Props>;

  function lazy<P, Component extends React.ComponentType<P>>(
      importFn: () => Promise<Component|{default: Component}>): Component;
  ...
  type StateUpdateFunction<State> =
      (newState: State|((oldState: State) => State)) => void;

  function useState<State>(initialState: State|
                           (() => State)): [State, StateUpdateFunction<State>];
  ...
  function useReducer<State, Action>(
      reducer: Reducer<State, Action>, initialState: State,
      initialAction?: Action): [State, (action: Action) => void];

  function useCallback<Callback extends Function, R>(
      f: Callback, keys?: any[]): Callback;
  ...
  function useImperativeMethods<Ref, ImperativeMethods>(
      ref: React.Ref<Ref>|undefined, f: () => ImperativeMethods,
      keys?: any[]): void;
}
